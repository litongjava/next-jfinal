...
import com.jfinal.aop.annotation.Import;
...
@Slf4j
public class JFinalApplicationContext implements Context {
  ...
  public Context run(Class<?>[] primarySources, String[] args) {
    ...
    TioBootServerHandler serverHandler = new TioBootServerHandler(wsServerConfig, defaultWebScoketHanlder, httpConfig,
        jfinalHttpRequestHandler, serverTcpHandler);
    ...
    if (requestHandler != null) {
      ...
      serverTioConfig.setAttribute(TioConfigKey.HTTP_REQ_HANDLER, jfinalHttpRequestHandler);
    }
    ...
    JFinalConfig jfinalConfig = Aop.get(JFinalConfig.class);
    if (jfinalConfig != null) {
      log.info("jfinal config:{}", jfinalConfig);
      ServletContext servletContext = new ServletContext();
      servletContext.setContextPath(httpConfig.getContextPath());
      jfinal = JFinal.me();
      jfinal.init(jfinalConfig, servletContext);
      Handler handler = jfinal.getHandler();
      if(jfinalHttpRequestHandler!=null) {
        jfinalHttpRequestHandler.setHandler(handler);
      }
