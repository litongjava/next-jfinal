...
import com.jfinal.commons.fileupload.disk.DiskFileItemFactory;
import com.jfinal.commons.fileupload.servlet.ServletFileUpload;
...
public class ProgressUploadFileKit {
  public static UploadFile get(HttpServletRequest request, String parameterName, String uploadPath,
      Consumer<UploadProgress> callback) {
    // 检查请求是否包含文件上传
    ...
    try {
      ...
      if (formItems != null && !formItems.isEmpty()) {
        ...
        if (StrKit.isBlank(parameterName)) {
          ...
          fileItem = formItems.stream()
              .filter(item -> (!item.isFormField() && parameterName.equals(item.getFieldName()))).findFirst()
              .orElse(null);
        }
        if (fileItem != null) {
          // 处理上传的文件
          ...
          if (isSafeFile(originFileName)) {
            ...
            progressFile = new UploadFile(parameterName, finalUploadPath, storeFile.getName(), originFileName,
